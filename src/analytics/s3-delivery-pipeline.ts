...
import { ArnFormat, Aws, CfnOutput, CfnResource, RemovalPolicy, Stack } from 'aws-cdk-lib';
...
export class S3DeliveryPipeline extends Construct {
  ...
  constructor(scope: Construct, id: string, props: DeliveryPipelineProps) {
      ...
      [
        {
          ...
          reason: 'service-role/AWSGlueServiceRole managed policy is being used for the  crawler.',
        },
        {
          id: 'AwsSolutions-IAM5',
          reason: 'service-role/AWSGlueServiceRole policy is using \'*\' in the policies it defines.',
        },
      ],
      true,
    );
    ...
    crawler.addDependsOn(this.processedTable);
    crawler.addDependsOn(this.rawTable);
    ...
    NagSuppressions.addResourceSuppressions(
      firehoseS3WriterRole,
      [
        {
          id: 'AwsSolutions-IAM5',
          reason: 'Generated when using CDK grantWrite and grantEncrypt methods on bucket and kms keys respectively.',
        },
      ],
      true,
    );
      ...
      [
        {
          ...
          reason: 'KDF will be enrypted as long as KMS key is passed in props and its a DIRECT_PUT and not from Delivery Stream.',
        },
      ],
      true,
    );
